            ...
            @foreach ($category as $index => $category)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap flex">
                        <button onclick="window.location='{{ route('edit_category', ['id' => $category->id]) }}'" class="p-2 focus:outline-none">
                        ...
                        <form action="{{ route('delete_category', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');">
                            @csrf
                            @method('DELETE')
